...
class Solution
{
    public:
    map<Node *, int> vis;
    void Helper(Node *root, int k, vector<Node *> &vec, int &count, int &an)
    {
        if(root==NULL) return;
        vec.push_back(root);
        count++;
        Helper(root->left, k, vec, count, an);
        Helper(root->right, k, vec, count, an);
        if(root->left==NULL and root->right==NULL and vec.size()-1>=k)
        {
            if(vis[vec[vec.size()-k-1]]==0)
            {
                vis[vec[vec.size()-k-1]]++;
                an++;
            }
        }
        vec.pop_back();
